...
# ----created by Ankur Karmakar--------------#
# ----modules: PyQt and Sqlite3-------------#


import sys, sqlite3, time
from PyQt6 import QtGui
from PyQt6.QtWidgets import QTableWidgetItem, QTableWidget, QComboBox, QVBoxLayout, QGridLayout, QDialog, QWidget, \
    QPushButton, QApplication, QMainWindow, QMessageBox, QLabel, QTextEdit, QProgressBar, QLineEdit, QDialogButtonBox
...
# DB has 4 tables among which only two has rows and columns in it currently. as you expand the application and
...
class DBHelper():
    def __init__(self):
        ...
        self.c.execute(
            "CREATE TABLE IF NOT EXISTS students(roll INTEGER,name TEXT,gender INTEGER,branch INTEGER,year INTEGER,academic_year INTEGER,address TEXT,mobile INTEGER)")
        self.c.execute(
            "CREATE TABLE IF NOT EXISTS payments(reciept_no INTEGER,roll INTEGER,fee INTEGER,semester INTEGER,reciept_date TEXT)")
    ...
    def addStudent(self, roll, name, gender, branch, year, academic_year, address, mobile):
        try:
            self.c.execute(
                "INSERT INTO students (roll,name,gender,branch,year,academic_year,address,mobile) VALUES (?,?,?,?,?,?,?,?)",
    ...
    def addPayment(self, roll, fee, semester):
        ...
        try:
            ...
            if not self.c.fetchone():
                ...
                if len(self.data) == 2:
                    QMessageBox.warning(QMessageBox(), 'Error',
                                        'Student with roll no ' + str(roll) + ' has already paid both semester fees.')
                ...
                # otherwise make the payment.
                    ...
                    self.c.execute("SELECT * from payments WHERE roll=" + str(roll) + " AND semester=0")
                    if not self.c.fetchone():
                        QMessageBox.warning(QMessageBox(), 'Error', 'Student with roll no ' + str(
                            roll) + ' has Odd Semester fee payment due.Pay that first.')
                    else:
                        self.c.execute(
                            "INSERT INTO payments (reciept_no,roll,fee,semester,reciept_date) VALUES (?,?,?,?,?)",
                ...
                # here we try to check if admin is trying to make payment for the same semester twice.
                    ...
                    QMessageBox.warning(QMessageBox(), 'Error',
                                        'Student with roll no ' + str(roll) + ' has already paid this semester fees.')
...
class AddStudent(QDialog):
    ...
    def addStudent(self):
        ...
        self.dbhelper.addStudent(self.roll, self.name, self.gender, self.branch, self.year, self.academic_year,
...
class Window(QMainWindow):
    ...
    def showStudentPayment(self):
        if not self.editFieldPayment.text() :
            QMessageBox.warning(QMessageBox(), 'Error',
                                'You must give the roll number to show the results for.')
            return None
        showstudent = DBHelper()
